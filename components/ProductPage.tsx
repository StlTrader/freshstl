...
import { Product, Review, Purchase } from '../types';
...
export const ProductPage: React.FC<ProductPageProps> = ({ product, onBack, onAddToCart, onCustomize, user, purchases = [] }) => {
    ...
    // Use previewStoragePath (GLB) if available, otherwise fallback to modelUrl
    // If previewStoragePath is a path (not URL), we need to get the URL? 
    // Wait, uploadFile returns URL for public files. 
    // In AdminPanel, we set previewStoragePath = path AND modelUrl = url.
    // So modelUrl should be the GLB URL if uploaded via new Admin.
    // But let's prefer previewStoragePath if it's a full URL or we can construct it.
    // Actually, AdminPanel sets modelUrl to the GLB URL. So product.modelUrl is likely the GLB.
    // But let's check if previewStoragePath is set, just in case.
    // For now, relying on modelUrl is safer if AdminPanel sets it correctly.
    // But wait, if I want to use the "preview" specifically...
    // Let's use modelUrl as the primary source for the viewer.
    ...
    const handleSecureDownload = async () => {
        ...
        try {
            ...
            link.href = url;
            link.download = `${product.name.replace(/\s+/g, '_')}.stl`; // Suggest filename
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <div className="bg-gray-900 rounded-2xl overflow-hidden shadow-2xl aspect-[4/3] lg:aspect-[16/9] relative ring-1 ring-white/10">
                        ...
                        <div className="lg:hidden">
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-500">
                                <span className="bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400 px-2 py-0.5 rounded-full font-bold text-xs uppercase tracking-wider">
                                    {product.category}
                                </span>
                                {product.aiModel && (
                                    <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 px-2 py-0.5 rounded-full font-bold text-xs uppercase tracking-wider flex items-center gap-1">
                                        <Sparkles size={10} /> {product.aiModel}
                                    </span>
                                )}
                                                ...
                                                <div key={review.id} className="flex gap-4">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="flex mb-2">
                                                            {[1, 2, 3, 4, 5].map(i => (
                                                                <Star key={i} size={14} className={i <= review.rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300 dark:text-dark-text-secondary"} />
                                                            ))}
                                                        </div>
                                            ...
                                            <form onSubmit={handleSubmitReview}>
                                                <div className="flex gap-2 mb-4">
                                                    {[1, 2, 3, 4, 5].map(star => (
                                                        ...
                                                        >
                                                            <Star size={24} className={star <= rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300 dark:text-dark-text-secondary"} />
                                                        </button>
                                                    ))}
                                                </div>
                                                <textarea
                        ...
                        <div className="sticky top-24 space-y-6">
                            ...
                            <div className="bg-white dark:bg-dark-surface rounded-2xl p-6 md:p-8 shadow-lg border border-gray-200 dark:border-dark-border">
                                <div className="hidden lg:block">
                                    ...
                                    {product.aiModel && (
                                        <span className="text-purple-600 dark:text-purple-400 font-bold text-xs uppercase tracking-widest mb-2 block flex items-center gap-1">
                                            <Sparkles size={12} /> Generated with {product.aiModel}
                                        </span>
                                    )}
                                    ...
                                    <div className="flex items-center gap-2 mb-6">
                                        <div className="flex text-yellow-400">
                                            {[1, 2, 3, 4, 5].map(i => (
                                                <Star key={i} size={16} className={i <= (product.rating || 5) ? "fill-current" : "text-gray-300 dark:text-dark-text-secondary"} />
                                            ))}
                                        </div>
                                ...
                                <button
                                    onClick={() => isPurchased ? handleSecureDownload() : onAddToCart(product)}
