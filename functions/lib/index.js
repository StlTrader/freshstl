...
exports.getSecureStlUrl = functions.https.onCall(async (data, context) => {
    ...
    if (!productId) {
        throw new functions.https.HttpsError("invalid-argument", "The function must be called with one argument 'productId'.");
    }
    // Verify Purchase
    ...
    if (!hasPurchased && !isAdmin) {
        throw new functions.https.HttpsError("permission-denied", "You must purchase this product to download it.");
    }
    ...
    const sourcePath = productData === null || productData === void 0 ? void 0 : productData.sourceStoragePath;
    if (!sourcePath) {
